...
use super::{
    datetime_to_database, map_sqlx_error, optional_datetime_to_database, optional_json_to_database,
    refresh_token_status_to_database, row_to_refresh_token_record, row_to_session,
...
impl SessionRepository for SqliteSessionRepository {
    ...
    async fn exchange_refresh(
        ...
        if existing.status == RefreshTokenStatus::Consumed {
            ...
            .bind(uuid_to_database(existing.refresh_token_id))
            .bind(datetime_to_database(now))
            .bind(uuid_to_database(existing.token_family_id))
        ...
        .bind(uuid_to_database(existing.session_id))
        .fetch_optional(&mut *transaction)
        .await
        .map_err(map_sqlx_error)?;
    ...
    async fn rotate_all_user_refresh_families(
        ...
        .bind(uuid_to_database(current_session_id))
        .fetch_optional(&mut *transaction)
        .await
        .map_err(map_sqlx_error)?;
        let session = session_row
...
async fn insert_session(
    ...
    .bind(uuid_to_database(session.session_id))
    .bind(uuid_to_database(session.internal_user_id))
    .bind(&session.provider_name)
    .bind(&session.client_id)
    .bind(optional_json_to_database(&session.device_metadata))
    .bind(session_status_to_database(&session.status))
    .bind(datetime_to_database(session.issued_at))
    .bind(datetime_to_database(session.expires_at))
    .bind(optional_datetime_to_database(session.revoked_at))
    .execute(&mut **transaction)
    .await
    .map_err(map_sqlx_error)?;
...
pub(crate) async fn insert_refresh_token(
    ...
    .bind(uuid_to_database(refresh_token.refresh_token_id))
    .bind(uuid_to_database(refresh_token.session_id))
    .bind(uuid_to_database(refresh_token.internal_user_id))
    .bind(uuid_to_database(refresh_token.token_family_id))
    .bind(&refresh_token.token_hash)
    .bind(refresh_token_status_to_database(&refresh_token.status))
    .bind(datetime_to_database(refresh_token.issued_at))
    .bind(datetime_to_database(refresh_token.expires_at))
    .bind(optional_datetime_to_database(refresh_token.consumed_at))
    .bind(optional_datetime_to_database(refresh_token.revoked_at))
    .execute(&mut **transaction)
    .await
    .map_err(map_sqlx_error)?;
